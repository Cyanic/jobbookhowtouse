# 'Time Inspector' Job Book Report

## Background

Job Book contains a number of existing reports that help staff review time that has been entered into the system:

1. Time Card Report
  * User selects a time range and clicks 'Load Data'
  * A table is displayed with the following columns:
    * Date (clickable, which takes the user to the relevant Daily Work Record)
    * Job #
    * Client
    * Line Item
    * DWR #
    * Employee Name
    * Start (time card start time)
    * End (time card end time)
    * Hours
    * Status (approval status)
    * Office
  * Implemented as a Kendo UI (jQuery) grid, columns are groupable and filterable

2. Time Grid
  * User selects a time range and clicks 'Load Data'
  * A table is displayed with the employee as the first column, then a column for each date in the time range (dynamic column configuration), then a 'total' column
  * Each 'date' column for the employee contains the total hours for that employee for that date, aggregated across all projects and line items

3. My Time Cards
  * Similar to 'Time Card Report' but only shows data for the current employee
